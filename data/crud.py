...
def add_details(db: Session, details: schemas.Detail):
    ...
    db_details = models.Detail(type=details.type, main_muscle=details.main_muscle, equipment=details.equipment, exercise_id=details.exercise_id)
    db.add(db_details)
    db.commit()
...
def add_media(db: Session, media: schemas.Media):
    ...
    db_media = models.Media(figure_img=media.figure_img, figure_url=media.figure_url, video=media.video, exercise_id=media.exercise_id)
    db.add(db_media)
    db.commit()
    db.refresh(db_media)
    return db_media
